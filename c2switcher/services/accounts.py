...
class AccountService:
    ...
    def set_nickname(self, identifier: str, nickname: str) -> 'Account':
        """
        Set or update nickname for an account.

        Returns the updated Account object.
        """
        account = self.get_account(identifier)
        self.store.update_nickname(account.uuid, nickname or None)
        return self.store.get_account_by_uuid(account.uuid)

    def remove_account(self, identifier: str) -> 'Account':
        """
        Remove account from database.

        Ends active sessions and reindexes remaining accounts.
        Returns the removed Account object.
        """
        account = self.get_account(identifier)

        ...
        try:
            ...
            if exc.response is not None and exc.response.status_code in (401, 403):
                ...
                refreshed_creds = self.credential_store.refresh_access_token(
                    account.credentials_json, force=True
                )
                token = refreshed_creds.get('claudeAiOauth', {}).get('accessToken')
                usage_data = ClaudeAPI.get_usage(token)
            elif exc.response is not None and exc.response.status_code == 429:
                # Rate limited — validate refresh token so dead accounts
                # surface as InvalidGrant instead of staying in limbo
                self.credential_store.validate_refresh_token(account.credentials_json)
                raise
            else:
                raise

...
class Store:
    ...
    def _create_schema(self):
        ...
        # Migrate existing databases that don't have the needs_reauth column
        try:
            cursor.execute('ALTER TABLE accounts ADD COLUMN needs_reauth BOOLEAN DEFAULT 0')
            self.conn.commit()
        except Exception:
    ...
    def delete_account(self, account_uuid: str):
        ...
        with self.conn:
            ...
            for new_idx, uid in enumerate(uuids):
                cursor.execute('UPDATE accounts SET index_num = ? WHERE uuid = ?', (new_idx, uid))
        self._load_all_caches()

    def reorder_accounts(self, ordered_uuids: List[str]):
        """Reorder accounts by assigning new index_nums based on given UUID order."""
        with self.conn:
            cursor = self.conn.cursor()
            # Phase 1: shift to negative temps to avoid UNIQUE conflicts during swap
            for temp_idx, uuid in enumerate(ordered_uuids):
                cursor.execute(
                    'UPDATE accounts SET index_num = ? WHERE uuid = ?',
                    (-(temp_idx + 1), uuid),
                )
            # Phase 2: assign final indices
            for new_idx, uuid in enumerate(ordered_uuids):
                cursor.execute(
                    'UPDATE accounts SET index_num = ?, updated_at = CURRENT_TIMESTAMP WHERE uuid = ?',
                    (new_idx, uuid),
                )
        self._load_all_caches()
    ...
    def update_nickname(self, account_uuid: str, nickname: Optional[str]):
        """Update account nickname."""
        with self.conn:
            cursor = self.conn.cursor()
            cursor.execute(
                'UPDATE accounts SET nickname = ?, updated_at = CURRENT_TIMESTAMP WHERE uuid = ?',
                (nickname, account_uuid),
            )
        self._load_accounts_cache()
    ...
    def set_needs_reauth(self, account_uuid: str, value: bool):
        ...
        with self.conn:
            cursor = self.conn.cursor()
            cursor.execute(
                'UPDATE accounts SET needs_reauth = ?, updated_at = CURRENT_TIMESTAMP WHERE uuid = ?',
                (1 if value else 0, account_uuid),
            )
        self._load_accounts_cache()
